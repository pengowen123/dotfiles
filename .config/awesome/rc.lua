...
beautiful.init("/home/pengowen/.config/awesome/theme.lua")
...
color_callback = function(widget, stdout)
    widget:set_markup("<span foreground='#FFFFFF'>" .. stdout .. "</span>")
end
...
-- Memory usage
memory = awful.widget.watch('bash -c "grep \\"MemTotal\\|MemFree\\|Buffers\\|Cached\\|MemAvailable\\" /proc/meminfo | awk \'{print $2}\'"', 5, function(widget, stdout)
    local total = nil
    local free = nil
    local buffers = nil
    local cached = nil
    ...
    format_num = function(n)
        return tonumber(string.format("%.1f", n / (1024 * 1024)))
    end

    local used_text = tostring(format_num(used)) .. " GiB"
    local available_text = tostring(format_num(available)) .. " GiB"

    local percent_free = available / total

    if percent_free < 0.05 then
        used_text = "<span foreground='#FF0000'>" .. used_text .. "</span>"
        available_text = "<span foreground='#FF0000'>" .. available_text .. "</span>"
    elseif percent_free < 0.1 then
        used_text = "<span foreground='#FFFF00'>" .. used_text .. "</span>"
        available_text = "<span foreground='#FFFF00'>" .. available_text .. "</span>"
    else
        used_text = "<span foreground='#FFFFFF'>" .. used_text .. "</span>"
...
awful.rules.rules = {
    ...
    -- Add titlebar to firefox
    { rule_any = { class = { "Nightly", "firefox" }
